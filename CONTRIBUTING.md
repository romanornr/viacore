Contributing to Viacore
=======

We're working hard to make *viacore* the most powerful JavaScript library for working with viacoin. Our goal is to have *viacore* be a library that can be used by anyone interested in viacore, and to level expertise differences with great design and documentation.
...
These are some global design goals in viacore that any change must adhere.
...
When possible, viacore objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of Viacore methods. For example, calling `JSON.stringify` on an Viacore object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'viacore' and Look up Classes from There
...
```javascript
var viacore = require('../');
var PublicKey = viacore.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add viacore git@github.com:viacore/viacore.git
git pull --rebase viacore master
```
...
```
Finally go to [github.com/viacoin/viacore](https://github.com/viacore/viacore) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of viacore.

If you have any questions feel free to post them to
[github.com/viacoin/viacore/issues](https://github.com/viacoin/viacore/issues).
